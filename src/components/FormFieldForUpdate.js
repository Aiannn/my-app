...
class FormFieldForUpdate extends Component {
    state = {
        first_name: '',
        last_name: '',
        email: '',
    ...
    handleSubmit = (e) => {
        ...
        fetch(`http://localhost:3000/users/${this.props.id}`, obj)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Failed to update user.') //didn't have a time to specify the error
                }
                return response.json()
            })
            .then(data => {
                alert(`User has been updated with id${data.id}`)
                this.setState({
                    first_name: '',
                    last_name: '',
                    email: '',
...
export default FormFieldForUpdate